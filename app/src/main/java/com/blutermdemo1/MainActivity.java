...
//    private TextView messagesTv;
    ...
    private EditText numberOfTimesEt;
    LinearLayout toplay;
    ...
    private void findViews() {
        ...
        sentBtn = (Button) findViewById(R.id.send_bt);
        stopBtn = (Button) findViewById(R.id.stop_bt);
        delayEt = (EditText) findViewById(R.id.delay_et);
        numberOfTimesEt = (EditText) findViewById(R.id.number_et);
        messagesEt = (EditText) findViewById(R.id.message_et);
    ...
    public void onClick(View v) {
        if (v == sentBtn) {
            ...
            delayEt.setError(null);
            messagesEt.setError(null);
            numberOfTimesEt.setError(null);

            String delay = delayEt.getText().toString();
            String msg = messagesEt.getText().toString();
            String number = numberOfTimesEt.getText().toString();

            if (TextUtils.isEmpty(delay) && TextUtils.isEmpty(msg)) {
                delayEt.setError("Field Required");
                delayEt.requestFocus();
            } else if (delay.length() == 0) {
                delayEt.setError("Field Required");
                delayEt.requestFocus();
            }else if (number.length() == 0) {
                numberOfTimesEt.setError("Field Required");
                numberOfTimesEt.requestFocus();
            }  else if (msg.length() == 0) {
                messagesEt.setError("Field Required");
                messagesEt.requestFocus();
            } else {
     ...
     */
    public void sendMessage(int delay, final int num, final String message) {
        ...
        if (message.length() > 0) {
            ...
            timer = new Timer();
            TimerTask onceADayTask = new TimerTask() {
                @Override
                public void run() {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            mEmulatorView.write(send, send.length);
                        }
                    });
            ...
            timer.schedule(onceADayTask, 0l, delay);
    ...
    private final Handler mHandlerBT = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MESSAGE_READ:
                ...
                if (readMessage.equalsIgnoreCase("#")) {
                    String splMsg = "##";
                    byte[] send = splMsg.getBytes();
                    mSerialService.write(send);
                    mEmulatorView.write(send, send.length);
                } else if (readMessage.equalsIgnoreCase("##")) {
                    String splMsg = "###";
                    byte[] send = splMsg.getBytes();
                    mSerialService.write(send);
                    mEmulatorView.write(send, send.length);
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (MainActivity.LOG_CHARACTERS_FLAG) {
                    ...
                    Log.w(MainActivity.LOG_TAG, "'" + Character.toString(printableB) + "' (" + Integer.toString(b) + ")");
                }
...
//        a.recycle();
     ...
     *
     */
    public void initialize(MainActivity blueTerm) {
        mBlueTerm = blueTerm;
        mTextSize = 15;
...
//            int h = visibleRect.height() - mBlueTerm.getTitleHeight() - 2;
            int h = visibleRect.height() - mBlueTerm.toplay.getHeight() - 300;
